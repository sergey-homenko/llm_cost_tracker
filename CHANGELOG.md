...
- OpenAI Realtime turns that reuse cached audio are no longer billed for it twice. Realtime counts cached audio inside both `audio_tokens` and `cached_tokens`, and the gem charged it in both places — at the cache-read rate and again at the full audio input rate — while the text input it had subtracted twice dropped out of the ledger. A `gpt-realtime` turn with 10,000 input tokens (2,000 text and 8,000 audio, 5,000 of the audio cached) was recorded as 13,000 input tokens costing $0.258 instead of 10,000 tokens costing $0.106. The per-modality breakdown in `cached_tokens_details` now takes cached audio and cached image tokens out of the audio and image buckets, so every input token is counted once. Cached tokens of every modality are billed at the model's cache-read rate: that matches OpenAI's published cached-audio price on the full-size `gpt-realtime` models, but sits below it on the mini models, which charge more for cached audio than for cached text. When an OpenAI-shaped usage block from any provider reports cached tokens without the breakdown and its buckets add up to more than the reported input, the excess is taken out of audio input first, then image input.
