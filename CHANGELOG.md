...
- Anthropic `data_residency` mode triggers on `inference_geo: "us"` only — the value Anthropic's API documents as their +1.1x uplift tier. Earlier preview ranges that listed `"eu"` were incorrect; EU data residency runs through Bedrock Frankfurt or Vertex Belgium with separate pricing, not the Anthropic API.
...
- OpenAI `web_search_call` is now priced model-aware. When the request used the legacy `web_search_preview` tool, the line item is recorded as `web_search_preview_request_reasoning` ($10/1k for gpt-5/o-series) or `web_search_preview_request_non_reasoning` ($25/1k for everything else), matching OpenAI's three published web-search billing paths. The OpenAI scraper extracts all three rates from the pricing page; rows that aren't published yet are silently skipped. `gpt-5-chat-latest` is explicitly classified as non-reasoning (despite the `gpt-5-` prefix) so its preview-tool calls bill at the higher non-reasoning rate.
- `Reconciliation::Sources::AnthropicUsage` pricing-mode classifier now matches the SDK integration and Faraday parser: Anthropic Priority Tier is treated as standard pricing (throughput tier, no per-token surcharge) and `data_residency` only triggers on `inference_geo: "us"` (the documented uplift), not on any non-empty geo. Previously reconciliation metadata could record `pricing_mode: "data_residency_priority"` even when the runtime ledger correctly said standard, creating attribution drift.
