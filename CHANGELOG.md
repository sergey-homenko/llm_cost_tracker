...
- `bin/rails llm_cost_tracker:backfill_unknown_pricing` rake task recomputes `total_cost`, `pricing_snapshot`, line-item costs, and rollup buckets for calls that landed with no pricing (e.g. a new model recorded before the next scraper refresh added its rates). The Data Quality dashboard's "Unknown pricing by model" panel now points at this task. Idempotent — re-running only touches calls still missing `total_cost`.
- `bin/rails generate llm_cost_tracker:upgrade_call_line_items_currency_index` writes a migration that adds a `(llm_cost_tracker_call_id, currency)` composite index on `llm_cost_tracker_call_line_items` (concurrent build on PostgreSQL) so the `Reconciliation::Diff` EXISTS subquery hits an index instead of a per-row scan. New installs already include the index.
- `bin/rails generate llm_cost_tracker:upgrade_provider_invoice_imports_finished_at_index` writes a migration that adds a `(state, finished_at)` index on `llm_cost_tracker_provider_invoice_imports` so `Retention.prune_invoice_imports` no longer sequentially scans the table on large installs. New installs already include the index.
- Azure OpenAI Service is captured out of the box. Faraday calls to `https://{resource}.openai.azure.com/openai/deployments/{deployment-id}/{operation}` and OpenAI Ruby SDK calls made with an Azure `base_url` are both tagged `provider: "azure_openai"`. Pricing resolves to the matching `openai/<model>` entry; regional / Data Zone uplifts are configurable via `config.pricing_overrides` with the `azure_openai/<model>` prefix. See [Configuration → Azure OpenAI Service](docs/configuration.md#azure-openai-service).
...
- `LlmCostTracker::Engine` is autoloaded, so `mount LlmCostTracker::Engine => "/llm-costs"` works without adding `require "llm_cost_tracker/engine"` to `config/application.rb`.
- `bin/rails generate llm_cost_tracker:upgrade_call_rollups_provider` and `upgrade_call_tags_key_value_index` build their indexes concurrently on PostgreSQL, so the upgrade migration no longer takes a long table-write lock on installs with millions of rows. The rollups upgrade keeps uniqueness enforced throughout the index swap and no longer wipes the existing `llm_cost_tracker_call_rollups` table when it adds the `provider` column — pre-upgrade aggregates stay (bucketed under empty provider) instead of being silently deleted on `db:migrate`.
- CSV exports stream calls in 500-row batches via offset pagination so peak memory stays flat at large export sizes while the user-selected sort order is preserved (`?sort=expensive`, `?sort=slow`, etc.).
...
- `llm_cost_tracker_ingestion_inbox_entries` declares its `payload` and `last_error` columns with `limit: 16.megabytes` so MySQL maps them to MEDIUMTEXT instead of TEXT, preventing silent truncation of large pricing snapshots and stack traces beyond the 64 KB TEXT cap. PostgreSQL is unaffected — `text` is unbounded there.
- Pricing now caches the `prices_file` mtime → ISO string conversion per stat, so each priced line item no longer triggers its own `File.mtime` syscall on the hot path.
- `LlmCostTracker::Ledger::Store` uses `INSERT ... RETURNING id, event_id` on adapters that support it (PostgreSQL, modern SQLite) instead of issuing a follow-up `SELECT` to recover the inserted call IDs for line-item and tag inserts. MySQL/Trilogy keeps the previous behaviour because RETURNING is not available there.
- `LlmCostTracker::Retention.prune` only plucks `pricing_snapshot` (and the other rollup columns) when `cache_rollups` is enabled. Default installs (`cache_rollups = false`) plucks only `:id`, dropping ~5-10 KB JSON per row times the batch size off the prune wire transfer.
- `Pricing::Unknown` no longer warns "unknown pricing for model X" when the call has no token pricing but at least one line item — including service charges — was successfully priced. Misleading false-positive is gone for Anthropic web-search-style calls on models missing from the token-pricing table.
- `Reconciliation::Diff#unmatched_local_calls_total_count` plucks attribution columns instead of materializing full Call records, dropping AR allocation + memory pressure on large reconciliation diffs.
