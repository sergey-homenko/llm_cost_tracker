...
- `bin/rails llm_cost_tracker:prices:refresh` refuses a price snapshot that sets an existing price to zero, charges for something that was free, removes a model's input or output rate, moves any price 100-fold or more, drops more than a third of the models, or switches currency, and leaves your pricing file as it was. The snapshot comes from the project's main branch and was only checked for well-formed, non-negative numbers, so one bad or malicious commit there could, on your next refresh, zero your prices so budgets never fire, or inflate them until `:block_requests` blocks every call. The error lists each suspicious change, `PREVIEW=1` prints the same list, `prices:check` prints it and fails, and `FORCE=1` (or `force: true`) writes the snapshot once you have confirmed it. The limits sit well outside what the bundled prices have ever done: the largest real move was about 55-fold, on an OpenRouter cache-read rate, and no refresh between bundled snapshots from v0.9 onward would have been stopped. It is a sanity check, not a signature: new model entries are not checked and a smaller deliberate change still gets through, so keep reviewing a refreshed file before you ship it.
- `bin/rails llm_cost_tracker:prices:refresh` refuses a price snapshot that zeroes an existing price or charges for a free one, drops a model's `input` or `output` rate, moves a price 100-fold or more, or switches currency, and leaves your pricing file untouched. Before, one bad commit to the snapshot on main could zero your prices so budgets never fire, or inflate them until `:block_requests` blocks every call. `PREVIEW=1` and `prices:check` list the suspicious changes; re-run with `FORCE=1` (or `force: true`) to accept them. It is a sanity check, not a signature: new models and smaller changes still get through, so keep reviewing the refreshed file.
- BREAKING: `pricing.unknown_model_behavior = :raise` records the call, with `cost_status: unknown`, before it raises `LlmCostTracker::UnknownPricingError`. It raised before the write, so the provider had already served and billed the request while the ledger lost it — the same ordering 0.14 fixed for `enforce_budget`. The error still reaches your code, now after the call is on record and subscribers are notified.
- `pricing.unknown_model_behavior = :raise` records the call, with `cost_status: unknown`, before it raises `LlmCostTracker::UnknownPricingError`. It raised before the write, so the provider had already served and billed the request while the ledger lost it — the same ordering 0.14 fixed for `enforce_budget`. The error still reaches your code, now after the call is on record and subscribers are notified.
- A malformed `pricing.file` fails `LlmCostTracker.configure` at boot instead of every LLM call, and the error says to fix the file or delete it and run `bin/rails llm_cost_tracker:prices:refresh`. A missing file is logged at boot and reported by `doctor` as an error, and calls are priced from `pricing.overrides` and bundled prices until `llm_cost_tracker:prices:refresh` creates it; before, every call raised and nothing was recorded.
- `pricing.unknown_model_behavior = :raise` records the call, with `cost_status: unknown`, before it raises `LlmCostTracker::UnknownPricingError`. It used to raise before the write, so a call the provider had already billed was missing from the ledger.
- A malformed `pricing.file` fails `LlmCostTracker.configure` at boot instead of every LLM call; fix it, or delete it and run `bin/rails llm_cost_tracker:prices:refresh`. A missing one is logged at boot and reported as an error by `doctor`, and calls use bundled prices until `prices:refresh` creates it; before, every call raised and nothing was recorded.
...
- The Pricing page no longer fails with a 500 when its `source` parameter is a list or a hash, as in `/pricing?source[]=bundled`; it shows the active price source instead, the same as for a source name it does not know. A list or hash in its `provider` filter is a bad request, as on the other dashboard pages.
- A crafted dashboard link can no longer point the dashboard's own links at another site. The dashboard copied every query parameter into its links, and Rails reads some of them as link options, so `?script_name=//evil.example` turned Export CSV, pagination, filter, tag, and "View" links into links to that site, `anchor`, `params`, `format`, `trailing_slash`, and `key` rewrote them in other ways, and a few such parameters made pages fail with a 500. Links and filter forms now carry only the dashboard's own parameters. Because every unknown parameter was also repeated in every link, a 50 KB junk parameter turned a tag page into a 10 MB response; that is gone too, and a query string over 16 KB — more than Puma, Unicorn, or nginx accept by default — is rejected as a bad request instead of being echoed into a page hundreds of times.
- `RubyLLM.transcribe` with a Gemini or Vertex AI model on RubyLLM 1.x is now recorded. RubyLLM 1.x sends these calls through its own Gemini transcription method, which bypasses the `RubyLLM::Provider#transcribe` the integration wraps, so they left no row.
- An LLM call captured automatically no longer fails because the gem could not record it. The Faraday middleware and the SDK integrations re-raised every `LlmCostTracker::Error` after the provider had answered — a missing price file, or the async inbox pool running out of connections after a five-second wait — so your code lost the response and the ledger lost the call. Only `BudgetExceededError`, `UnknownPricingError` under `:raise`, and `TransactionAbortedError` reach your code now; any other recording failure is logged. `LlmCostTracker.track` and `track_stream` still raise, since you called them to record.
- A recording error no longer replaces the exception your own code raised while iterating a captured OpenAI or Anthropic SDK stream or inside `track_stream`, including worker shutdown signals such as `Sidekiq::Shutdown`, and the interrupted stream is still recorded and tagged `stream_errored`; the recording error is logged instead. A lost host transaction still wins, and your exception stays in its cause chain.
- An OpenAI or Anthropic batch holding an unpriced model under `:raise`, or crossing a budget, records every result before raising once. `batches.retrieve` raised on every poll instead, and results after the failing line were never recorded.
- An automatically captured LLM call no longer fails because the gem could not record it, such as when the async inbox pool runs out of connections; your code lost the provider's response. Only `BudgetExceededError`, `UnknownPricingError` under `:raise`, and `TransactionAbortedError` reach your code; other recording failures are logged. `LlmCostTracker.track` and `track_stream` still raise them.
- A recording error no longer replaces an exception your code raised while iterating an OpenAI or Anthropic SDK stream or inside `track_stream`, including shutdown signals such as `Sidekiq::Shutdown`. The stream is still recorded, tagged `stream_errored`, and the recording error is logged. `TransactionAbortedError` still wins, with your exception as its cause.
- An OpenAI or Anthropic batch with an unpriced model under `:raise`, or one crossing a budget, records every result before raising once. Results after the failing one were never recorded, and `batches.retrieve` raised on every poll.
- With `ingestion.mode = :async`, a call the database cannot store no longer takes its whole batch with it. The worker wrote up to 100 inbox rows in one transaction, and when one of them failed it marked all of them failed; after five attempts every row was quarantined, which removed it from the ledger and from budget totals, so one bad tag value could drop 99 other calls and most of a month's spend. The worker now falls back to writing rows one at a time when a batch fails, including a batch too large for one MySQL statement or one that hits a statement timeout, stores every row the database accepts, and retries only the rejected rows, apart from new ones. A connection dropped or a statement timed out while writing a single row stops the drain without counting toward quarantine.
- A call is no longer lost because one of its strings cannot be stored. A NUL byte in a model, response id, tag value, or line item — which PostgreSQL rejects — invalid UTF-8, an identifier longer than a MySQL string column, or a token count above the integer column made `LlmCostTracker.track` raise and every automatic capture drop the call with a warning, and `LlmCostTracker.with_tags` raised on invalid UTF-8 before running its block. NUL bytes are now removed and invalid UTF-8 is replaced with U+FFFD before the call is priced, string columns are capped at 255 characters when the row is written, and token counts are capped at the column limit with a warning while the cost is still computed from the full counts. Inbox rows written by earlier releases with a NUL byte are cleaned the same way when they drain.
- With `ingestion.mode = :async`, one inbox row the database rejects no longer fails its whole batch. The worker marked every row of a failed batch failed, so after five attempts one bad row quarantined up to 99 other calls, dropping them from the ledger and budget totals. A failed batch is now written one row at a time: every row the database accepts lands, and only rejected rows are marked failed and logged. Rows already quarantined this way can be requeued as described in `docs/operations.md`.
- A NUL byte in a stored string, which PostgreSQL rejects, or invalid UTF-8 in a tag value no longer loses the call, and `LlmCostTracker.with_tags` no longer raises on invalid UTF-8. NUL bytes are removed when the row is written, including from inbox rows written by earlier releases, and invalid UTF-8 in tag values is replaced with U+FFFD.
- An interrupted Gemini or Azure stream records its model from the request URL instead of `unknown`.
- The async inbox keeps the error that stopped a row from being stored. A database error message cut at 1,000 bytes in the middle of a multibyte character could not be written, so `last_error` stayed empty.
- A dashboard link carrying hundreds of `tag[...]` filters can no longer take PostgreSQL down. Every tag filter adds a subquery that the planner weighs against all the others, so planning memory grew with the square of the filter count — about 80 MB at 100 filters and 330 MB at 200 — and a URL with around 700 of them got the Postgres backend killed for running out of memory, which restarted the whole database. Every dashboard page runs that query several times per request. A page now accepts at most 10 tag filters, counting the value on a tag value page, and answers anything more with a bad request before touching the database.
- The dashboard's tag, provider, model, stream, and usage-source filters handle bad input without a 500. A CSV export with an invalid filter, a missing call requested as CSV, and a database error during an export failed with a 500 or came back as an HTML page labelled as CSV, because the error pages existed only as HTML; they now render as HTML error pages with the right status. A list or hash in one of these filter values was silently turned into a string that matched nothing and is now rejected as an invalid filter. On PostgreSQL, a NUL byte inside a filter value raised a driver error; PostgreSQL cannot store such a value, so the filter now simply matches nothing. On a tag value page, a filter on the same tag key no longer contradicts the value being shown, and a tag breakdown already at the tag filter limit no longer offers drill-down links that go past it. Tag pages for keys containing a dot, such as `/tags/team.name` or `/tags/request.json`, load instead of returning 404 or 406, including the links the dashboard itself builds to them.
- An LLM call made inside your own database transaction no longer breaks that transaction when the ledger write fails. The gem swallowed the error, but on PostgreSQL the failed statement had already aborted your transaction, so your app's own writes were lost with `PG::InFailedSqlTransaction`. Ledger writes, budget reads, and batch de-duplication now run in a savepoint while a transaction is open. With `config.budgets.totals_source = :cache`, the rollup increment waits until your transaction commits on Rails 7.2 and later, so an open transaction no longer holds the rollup row lock and stalls every other request recording spend for the same provider; on Rails 7.1 and inside non-joinable transactions such as transactional test fixtures it still runs immediately, and it is never retried inside a transaction. On MySQL, where a deadlock rolls back your whole transaction, the gem now raises `LlmCostTracker::TransactionAbortedError` instead of swallowing the error and letting your code keep writing outside the transaction it thinks it is in.
- Long streams are recorded with their real usage instead of zero tokens and `$0`. Streams were buffered whole and given up at 1 MB, and the usage every provider sends at the end of the stream was exactly what got dropped — roughly 3,000 output tokens of chat completions through Faraday, a little more through the SDKs. Streams are now decoded as they arrive, and only the first and last events plus billable tool-call and grounding events are kept, so capture works at any length with flat memory. Image streams through Faraday no longer lose their final usage either.
...
- OpenAI Realtime cached audio is no longer billed twice. Realtime counts it in both `audio_tokens` and `cached_tokens`, so it was charged at the cache-read rate and again at the full audio rate (a 10,000-token `gpt-realtime` turn with 5,000 cached audio tokens cost $0.258 instead of $0.106). Cached audio and image tokens from `cached_tokens_details` now come out of the audio and image buckets and are priced at the model's cache-read rate, which is below OpenAI's published cached-audio price on the mini models. OpenAI-compatible usage without that breakdown whose buckets exceed the reported input has the overlap taken out of audio input, then image input.

### Security

- A provider API key sent as a URL parameter could be stored in the ledger. From v0.9.0 through v0.14.0, when a Faraday stream failed with an HTTP error raised inside the middleware — for example `f.response :raise_error` listed after `f.use :llm_cost_tracker`, on a Gemini `?key=` or Azure `?api-key=` URL — the `stream_interrupted_error` tag stored Faraday's error message, full request URL included, where it appeared on the dashboard and in CSV exports and went to notification subscribers. The tag now holds only the error class, and the HTTP status goes into a new `stream_interrupted_status` tag. Rotate any key that may have been exposed, and clear stored messages with `UPDATE llm_cost_tracker_call_tags SET value = split_part(value, ': ', 1) WHERE key = 'stream_interrupted_error'` on PostgreSQL, or ``UPDATE llm_cost_tracker_call_tags SET value = SUBSTRING_INDEX(value, ': ', 1) WHERE `key` = 'stream_interrupted_error'`` on MySQL. With `ingestion.mode = :async`, run it after the inbox has drained, and delete quarantined rows in `llm_cost_tracker_ingestion_inbox_entries` whose `payload` contains `stream_interrupted_error`.
- Credentials inside longer strings are scrubbed before they are stored, logged, or sent to subscribers. The tag sanitizer only caught a value that was entirely a key or token, so `key=`, `api-key=`, `token=`, or `sig=` URL parameters, URL user info, `Authorization` headers, and provider keys inside a longer tag value were stored as they were; they are now replaced with `[REDACTED]` in place. The same scrubbing now applies to the gem's log lines, to the async inbox's `last_error` column, which could echo row values from database errors, to `TransactionAbortedError` messages, and to the `source_url` that `llm_cost_tracker:prices:refresh` writes into your price file and prints.
