...
- Per-tag budgets with `:block_requests` check the tags passed to the Faraday middleware (`f.use :llm_cost_tracker, tags: ...`) before the call is sent. They saw only `with_tags` context, so a tenant over its budget was still sent to the provider and failed only after the spend.
- Transcriptions recorded through RubyLLM are priced at the model's audio input rate when it has one. RubyLLM reports only total input tokens, so a transcription's audio was priced as text: about a third of the real cost on `gemini-2.5-flash` and less than half on `gpt-4o-transcribe`.
...
- The async inbox's `last_error` is no longer left empty when the error message is cut at 1,000 bytes in the middle of a multibyte character.
