...
- `config.ingestion_adapter = :inline` (new default) writes events synchronously from the request thread; the durable inbox + worker + leases are now opt-in via `config.ingestion_adapter = :durable` plus `bin/rails generate llm_cost_tracker:durable_ingestion`. Default installs ship with two fewer tables (`llm_cost_tracker_ingestion_inbox_entries`, `llm_cost_tracker_ingestion_leases`).
- `config.maintain_rollups = false` (new default) drops the rollup write path. Budget reads aggregate live from `llm_cost_tracker_calls` instead. Apps that want the indexed rollup fast path opt in via `config.maintain_rollups = true` plus `bin/rails generate llm_cost_tracker:call_rollups`. Default installs ship with one fewer table (`llm_cost_tracker_call_rollups`).
...
- BREAKING: `config.ingestion_adapter` defaults to `:inline`. Tracking writes go directly to the ledger from the request thread; the durable inbox + worker + leases tables are no longer created by the install generator. Existing installs that want to keep the durable inbox path must set `config.ingestion_adapter = :durable` in the initializer (their existing `llm_cost_tracker_ingestion_*` tables remain compatible). Fresh installs that need durability run `bin/rails generate llm_cost_tracker:durable_ingestion` and set the adapter.
- BREAKING: `config.maintain_rollups` defaults to `false`. Budget reads aggregate live from `llm_cost_tracker_calls`; the rollup table is no longer created by the install generator. Existing installs that want fast budget reads on large ledgers must set `config.maintain_rollups = true` in the initializer (their existing `llm_cost_tracker_call_rollups` table remains compatible). Fresh installs run `bin/rails generate llm_cost_tracker:call_rollups` and set the flag.
