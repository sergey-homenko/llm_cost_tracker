...
- `budget_exceeded_behavior = :block_requests` now also estimates the current call's input cost via a character-count heuristic (chars / 4 ≈ tokens, provider-agnostic, no external tokenizer) and blocks before send when prior spend plus the estimate would cross the daily / monthly limit, or when the estimate alone crosses `per_call_budget`. Output tokens stay unknown pre-send and are caught by the existing post-record check. `BudgetExceededError` and `on_budget_exceeded` payloads gain a `stage` field (`:pre_send` or `:post_spend`). Unknown models (no pricing match) skip the estimate. See [Budgets](docs/budgets.md).
