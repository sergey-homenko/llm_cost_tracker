...
- Reconciliation `external_id` is namespaced by `source/provider` (e.g. `csv/openai:row-1`) when a single source carries multiple providers, so the same CSV row id imported as OpenAI and as Anthropic no longer collides on the unique index. Native sources (`openai:` / `anthropic:` / `gemini:`) keep their pre-existing prefix. Pre-release v0.9 deployers who imported CSV rows under the legacy `csv:` prefix should `LlmCostTracker::ProviderInvoice.delete_all` and re-import.
- OpenAI Cost API reconciliation tags the organization id under `provider_workspace_id` to match the calls schema (`Diff` scopes on that field). Pre-release v0.9 deployers who imported via OpenAI Cost API before this change have `provider_organization_id` in metadata and lose org-level scope; `delete_all` + re-import re-stamps the field correctly.
- Reconciliation dashboard groups latest-period and drill-down rows by `(source, provider, currency)`. A second provider importing under the same source (e.g. `csv` for both OpenAI and Anthropic) no longer hides its drift in the first provider's row.
- Reconciliation diff drill-down (unmatched provider rows, unmatched local calls, non-cost evidence) is capped at `drilldown_limit` (default 100) ranked by amount; the underlying `_total` counts come from independent SQL counts so they stay accurate even when the drill-down load is bounded. CLI: `DRILLDOWN_LIMIT=all rake llm_cost_tracker:reconcile:diff` to see every row.
- Reconciliation imports stamp `match_basis` automatically from the most specific attribution dimension present (project > api_key > workspace > model, falling back to `period_only`). The Ruby drill-down and the SQL count now agree even for CSV rows that omit the field.
- Reconciliation diff loads only the top-by-amount slice of invoices into Ruby (capped at `drilldown_limit * 5`); a million-row monthly reconciliation no longer materialises every row.
...
- Reconciliation install migration is re-runnable on installs that already carry the v0.8 `llm_cost_tracker_provider_invoices` placeholder tables (`create_table :…, if_not_exists: true` plus matching index guards).
- OpenAI hosted-tool service line items (`web_search_call`, `file_search_call`, `code_interpreter_call`, `mcp_call`) are captured when the SDK returns the type as a Symbol — the SDK declares `type, const: :web_search_call` and a previous string-keyed lookup silently dropped these.
- Interrupted Anthropic and Gemini streams record the right provider name instead of `provider: "unknown"`.
- Tag sanitizer redacts secrets before truncating, and the `[REDACTED]` marker stays whole even when `max_tag_value_bytesize` is smaller than 10 bytes.
- `Pricing::Registry` rejects non-finite price values (`Infinity` / `NaN`) alongside negatives.
- Period totals fall back to `llm_cost_tracker_calls` aggregation when `cache_rollups = true` but the rollups table has no row for the period (e.g. after the v0.9 `upgrade_call_rollups_provider` migration clears it). Budget reads and dashboard totals no longer read zero during the rebuild window.
- Stream collector drops `b64_json` / `partial_image_b64` payload fields plus any string field over 8 KB before counting buffer bytes, so OpenAI image generation and audio streams keep their final usage chunk instead of overflowing on the first base64 chunk.
...
- Inline ingestion documentation now matches the code: `Tracker.record` with `config.durable_ingestion = false` writes through the caller's ActiveRecord connection and is rolled back with the caller's transaction. Flip `durable_ingestion = true` for rollback isolation; that path uses a separate connection for inbox writes.
