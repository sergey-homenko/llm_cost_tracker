...
- BREAKING: the `pricing_snapshot` audit record (the `calls.pricing_snapshot` column and the notification payload) is now uniformly String-keyed with decimal-String rate `amount`s — the notification payload's keys were Symbols before, and token-rate `amount`s were JSON numbers. Parse it with String keys.
