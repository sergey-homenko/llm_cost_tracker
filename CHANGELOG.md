...
- OpenAI `gpt-image-1` / `gpt-image-1-mini` / `gpt-image-1.5` / `gpt-image-2` priced per image-token. The SDK integration extracts `usage.input_tokens_details.image_tokens` for image-as-input flows (edits / variations) and treats `usage.output_tokens` as image output. Requires the new `bin/rails generate llm_cost_tracker:upgrade_image_tokens` migration on v0.8 → v0.9 upgrades.
...
- `prices:refresh` merges scraped service-charge rates into the existing catalog instead of replacing the provider hash. Manually-curated entries survive a refresh.
...
- Anthropic Cost API parser reads `starting_at` / `ending_at` bucket bounds and the structured `cost_type` field (the previous `starts_at` / `ends_at` and description-based matching produced zero rows against live admin Cost API responses). `service_tier: "batch"` and `inference_geo: "us"` are the only fields that promote a row's `pricing_mode`.
...
- Header `total_cost` and per-line-item rates can no longer disagree on the context-tier boundary or the resolved pricing mode — both are derived from the same token usage in one pass.
