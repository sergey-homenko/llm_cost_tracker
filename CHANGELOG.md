...
- Gemini and Vertex AI transcriptions made through RubyLLM 1.x are recorded. RubyLLM 1.x runs them through its own Gemini transcription method, which never reaches the `RubyLLM::Provider#transcribe` the integration wraps, so `RubyLLM.transcribe` with a Gemini or Vertex AI model left no row while OpenAI transcriptions through the same call were recorded. These calls are now recorded once, with the input and output tokens Gemini reports.
