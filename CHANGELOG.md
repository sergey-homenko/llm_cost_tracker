...
- Dashboard tag detail page drills into a single value via `?value=…` with total cost, call count, average per call, and a daily spend timeseries.
- Bundled rates for OpenAI embeddings (`text-embedding-3-small` / `-3-large` / `-ada-002`, including 50% batch discount) and token-priced transcription (`gpt-4o-transcribe`, `gpt-4o-mini-transcribe`). Token-priced transcription splits audio and text inputs at their separate rates. DALL-E, Whisper, and TTS still record as zero-token visibility events until their per-image / per-minute / per-character pricing components land.
...
- `Pricing::Sync.refresh` preserves models flagged with `_source: "manual"` in the local file. Manual rates survive remote refreshes.
...
- `Pricing::Scrape::Orchestrator` merges scraped service-charge rates into the existing catalog instead of replacing the provider hash. Manually-curated entries survive `prices:refresh`.
...
- `Reconciliation::Sources::AnthropicUsage` matches the runtime classifier: Priority Tier as standard, `data_residency` only on `inference_geo: "us"`. Reconciliation metadata no longer drifts from the runtime ledger.
...
- `Capture::StreamCollector` no longer lets a request-level `pricing_mode` override what the provider reports. Provider-reported standard wins over a request that asked for priority.
- The new generators (`call_rollups`, `durable_ingestion`, `reconciliation`, `upgrade_call_rollups_provider`) are registered with the Railtie and reachable through `bin/rails generate`.
- Faraday streaming captures no longer silently degrade to `usage_source: :unknown` due to a missing `stringio` require.
...
- `Tracker.record` runs the pricing pipeline once per event instead of twice. Header cost, snapshot, and per-line-item rates now share a single `Pricing.calculate(provider:, model:, tokens:, line_items:, pricing_mode:)` pass anchored to `capture.token_usage`. Header totals and line-item rates can no longer disagree on the context-tier boundary or pricing mode resolution.
...
- Engine dashboard adds CSRF protection on the reconciliation import endpoint, sets `Cache-Control: no-store` on CSV exports, registers `tag` / `tag_value` in `config.filter_parameters`, and emits `X-Frame-Options: DENY` / `Referrer-Policy: same-origin` / a baseline `Content-Security-Policy` on every dashboard response. CSV export streams via `find_each` instead of buffering.
- Schema drift checks behind the dashboard now run once per process and invalidate via `Rails.application.reloader.to_prepare`. Per-request `connection.indexes` / `connection.columns` / `data_source_exists?` calls are gone; production runs the check on first dashboard hit and reuses the result, development re-checks on every code reload.
