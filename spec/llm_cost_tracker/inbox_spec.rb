...
RSpec.describe "ActiveRecord durable inbox" do
  before do
    ...
    LlmCostTracker::InboxEvent.reset_column_information
    LlmCostTracker::IngestorLease.reset_column_information
    LlmCostTracker::Inbox.reset!
    LlmCostTracker::LedgerStore.reset!

    allow(LlmCostTracker::Ingestor).to receive(:ensure_started)
  end
  ...
  it "captures events into a durable inbox before ingesting them into the ledger" do
    ...
    expect(LlmCostTracker::InboxEvent.count).to eq(1)
    expect(LlmCostTracker::LlmApiCall.count).to eq(0)
    expect(LlmCostTracker::InboxEvent.first.event_id).to eq(event.event_id)

    expect(LlmCostTracker.flush!).to be true

    call = LlmCostTracker::LlmApiCall.first
    expect(LlmCostTracker::InboxEvent.count).to eq(0)
  ...
  it "includes pending inbox costs in period totals before ingesting" do
    ...
    expect(LlmCostTracker::LedgerStore.monthly_total(time: Time.utc(2026, 4, 30, 23))).to eq(0.0025)

    LlmCostTracker.flush!

    expect(LlmCostTracker::InboxEvent.count).to eq(0)
    expect(LlmCostTracker::PeriodTotal.find_by!(period: "day", period_start: Date.new(2026, 4, 18)).total_cost.to_f)
      .to eq(0.0025)
    expect(LlmCostTracker::LedgerStore.daily_total(time: Time.utc(2026, 4, 18, 23))).to eq(0.0025)
  end
  ...
  it "reads stored and pending budget totals in one database statement" do
    ...
    sqls = []
    allow(LlmCostTracker::LlmApiCall.connection).to receive(:select_all).and_wrap_original do |method, *args, **kwargs|
      sql_text = args.first.to_s
      sqls << sql_text if sql_text.include?("llm_cost_tracker_inbox_events")
      method.call(*args, **kwargs)
    end

    expect(LlmCostTracker::LedgerStore.daily_total(time: time)).to eq(3.75)
  ...
  it "ingests unknown-cost events without adding pending budget totals" do
    ...
    expect(LlmCostTracker::InboxEvent.first.total_cost).to be_nil
    expect(LlmCostTracker::LedgerStore.daily_total(time: event.tracked_at)).to eq(0.0)

    LlmCostTracker.flush!

    expect(LlmCostTracker::LlmApiCall.find_by!(event_id: event.event_id).total_cost).to be_nil
  end
  ...
  it "does not double-count a retried inbox event that already reached the ledger" do
    ...
    row = LlmCostTracker::InboxEvent.first
    parsed = LlmCostTracker::Inbox.event_from_row(row)

    LlmCostTracker::LlmApiCall.transaction do
      LlmCostTracker::LedgerStore.insert_many([parsed])
    end
    LlmCostTracker.flush!

    expect(LlmCostTracker::LlmApiCall.where(event_id: event.event_id).count).to eq(1)
    expect(LlmCostTracker::PeriodTotal.find_by!(period: "day",
  ...
  it "does not increment rollups when a concurrent duplicate insert wins the race" do
    ...
    row = LlmCostTracker::InboxEvent.first
    parsed = LlmCostTracker::Inbox.event_from_row(row)
    allow(LlmCostTracker::LlmApiCall).to receive(:insert_all!).and_raise(ActiveRecord::RecordNotUnique)
    ...
    LlmCostTracker::InboxEvent.delete_all
  end

  it "allows one ingestor lease holder until the lease expires" do
    first = LlmCostTracker::IngestorLeaseClaim.new(identity: "worker-a", seconds: 10)
    second = LlmCostTracker::IngestorLeaseClaim.new(identity: "worker-b", seconds: 10)
  ...
  it "does not ingest while another worker owns the leader lease" do
    ...
    expect(LlmCostTracker::Ingestor.ingest_once).to eq(0)
    expect(LlmCostTracker::InboxEvent.count).to eq(1)

    LlmCostTracker::IngestorLease.find_by!(name: "default").update!(locked_until: Time.now.utc - 1)

    expect(LlmCostTracker::Ingestor.ingest_once).to eq(1)
    expect(LlmCostTracker::InboxEvent.count).to eq(0)
  end
  ...
  it "marks failed batches for retry" do
    ...
    allow(LlmCostTracker::LedgerStore).to receive(:insert_many).and_raise("write failed")
    allow(LlmCostTracker::Logging).to receive(:warn)

    expect(LlmCostTracker::Ingestor.ingest_once(require_lease: false)).to eq(0)
  ...
  it "quarantines invalid inbox rows without blocking valid rows behind them" do
    ...
    expect(LlmCostTracker::Ingestor.ingest_once(require_lease: false)).to eq(2)

    bad_row = LlmCostTracker::InboxEvent.find_by!(event_id: "bad-event")
    expect(bad_row.attempts).to eq(LlmCostTracker::Inbox::MAX_ATTEMPTS)
    expect(bad_row.last_error).to include("JSON")
    expect(LlmCostTracker::LlmApiCall.find_by!(event_id: event.event_id)).to be_present
    expect(LlmCostTracker.flush!(timeout: 0.01)).to be true
    expect(LlmCostTracker::InboxEvent.where(event_id: "bad-event")).to exist
  ...
  it "excludes quarantined inbox rows from pending budget totals" do
    ...
    LlmCostTracker::InboxEvent.create!(
      ...
      tracked_at: time,
      payload: "{",
      attempts: LlmCostTracker::Inbox::MAX_ATTEMPTS
    )

    expect(LlmCostTracker::LedgerStore.daily_total(time: time)).to eq(0.0)
  end
  ...
  it "times out flush when every row is leased by another worker" do
    ...
    LlmCostTracker::InboxEvent.update_all(locked_at: Time.now.utc, locked_by: "worker-a")
  ...
  it "keeps flushing after a processed inbox batch" do
    ingestor = LlmCostTracker::Ingestor
    batch = instance_double(LlmCostTracker::InboxBatch)
    allow(LlmCostTracker::InboxBatch).to receive(:new).and_return(batch)
  ...
  it "waits between empty flush attempts only while the deadline is still open" do
    ...
    expect(durations.first).to be <= LlmCostTracker::Ingestor::INTERVAL_SECONDS
  end
  ...
  it "does not start or flush the ingestor when durable inbox is disabled" do
    ...
    allow(LlmCostTracker::Inbox).to receive(:enabled?).and_return(false)
  ...
  it "starts and stops the ingestor thread lazily" do
    allow(LlmCostTracker::Ingestor).to receive(:ensure_started).and_wrap_original(&:call)
    LlmCostTracker::Inbox.reset!

    LlmCostTracker::Ingestor.ensure_started

    thread = LlmCostTracker::Ingestor.instance_variable_get(:@thread)
  ...
  it "reports a missing ActiveRecord table during verification" do
    allow(LlmCostTracker::LlmApiCall).to receive(:table_exists?).and_return(false)
  ...
  it "reports unexpected ActiveRecord verification failures" do
    allow(LlmCostTracker::LlmApiCall).to receive(:table_exists?).and_raise("schema failed")
  ...
  it "can capture through a separate connection when the caller has an open transaction" do
    ...
    expect(LlmCostTracker::InboxEvent.find_by!(event_id: event.event_id)).to be_present
  end

  it "fails honestly when no separate connection is available inside a caller transaction" do
    connection = LlmCostTracker::LlmApiCall.connection
    allow(connection).to receive(:transaction_open?).and_return(true)
    allow(LlmCostTracker::Inbox)
  ...
  it "returns false when shutdown cannot flush cleanly" do
    allow(LlmCostTracker::Ingestor).to receive(:flush!).and_raise("flush failed")
    allow(LlmCostTracker::Logging).to receive(:warn)

    expect(LlmCostTracker::Ingestor.shutdown!(timeout: 0.01)).to be false
  end

  it "passes shutdown timeout through the public API" do
    allow(LlmCostTracker::Ingestor).to receive(:shutdown!).and_return(true)
    expect(LlmCostTracker::Ingestor)
      .to receive(:shutdown!)
  ...
  it "can stop the ingestor without draining durable rows" do
    ...
    flush_calls = 0
    allow(LlmCostTracker::Ingestor).to receive(:flush!) { flush_calls += 1 }

    expect(LlmCostTracker::Ingestor.shutdown!(timeout: 0.01, drain: false)).to be true
  ...
  it "uses the leader lease when shutdown drains" do
    flush_calls = []
    allow(LlmCostTracker::Ingestor).to receive(:flush!) do |**kwargs|
      flush_calls << kwargs
      true
    end

    expect(LlmCostTracker::Ingestor.shutdown!(timeout: 0.01)).to be true
  ...
  it "does not acquire a leader lease while the inbox is empty" do
    ...
    generation = 1
    ingestor.instance_variable_set(:@generation, generation)
    batch = instance_double(LlmCostTracker::InboxBatch, claimable?: false, pending?: false)
    allow(LlmCostTracker::InboxBatch).to receive(:new).and_return(batch)
    allow(ingestor).to receive(:sleep) { ingestor.instance_variable_set(:@stop_requested, true) }
    allow(LlmCostTracker::IngestorLeaseClaim).to receive(:new)

    ingestor.instance_variable_set(:@stop_requested, false)
    ingestor.send(:run, generation)

    expect(LlmCostTracker::IngestorLeaseClaim).not_to have_received(:new)
  end
  ...
  it "warns when ingestor storage errors happen" do
    ...
    LlmCostTracker::Ingestor.send(:handle_error, RuntimeError.new("boom"))
  ...
  it "ignores wakeup races for threads that already stopped" do
    ...
    expect do
      LlmCostTracker::Ingestor.send(:wake_thread, thread)
    end.not_to raise_error
  end

  it "ignores failures while marking failed rows" do
    allow(LlmCostTracker::InboxEvent).to receive(:where).and_raise("write failed")
    batch = LlmCostTracker::InboxBatch.new(identity: "test")

    expect do
      batch.mark_failed(
        [instance_double(LlmCostTracker::InboxEvent, id: 1)],
