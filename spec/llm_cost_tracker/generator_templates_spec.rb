...
RSpec.describe "generator templates" do
  ...
  it "creates calls, line items, tags, and ingestion tables for PostgreSQL installs" do
    ...
    expect(migration).not_to include("add_index :llm_cost_tracker_calls, [:provider, :tracked_at]")
    expect(migration).not_to include("add_index :llm_cost_tracker_calls, [:model, :tracked_at]")
  ...
  it "provides a complete initializer template" do
    ...
    expect(initializer).to include("config.pricing.unknown_model_behavior = :warn")
    ...
    expect(initializer).to include("# config.capture.openai_compatible_providers")
    expect(initializer).to include("# config.tags.report_breakdown_keys")
  ...
  describe "upgrade_call_rollups_provider generator" do
    it "writes a guarded provider-column migration" do
      Dir.mktmpdir do |dir|
        ...
        expect(migration).to include("add_unique_index NEW_INDEX")
        expect(migration).to include("remove_index TABLE, column: OLD_INDEX, unique: true, if_exists: true")
