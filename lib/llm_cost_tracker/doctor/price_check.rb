  ...
  class Doctor
    class PriceCheck
      ...
      def call
        ...
        return missing_file_check(path) unless File.exist?(path)
      ...
      def missing_file_check(path)
        Check.new(:error, "prices", "pricing.file #{path} does not exist; #{REFRESH_COMMAND}")
      end
