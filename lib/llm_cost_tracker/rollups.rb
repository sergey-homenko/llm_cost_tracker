...
require_relative "rollup_batch"
require_relative "rollup_upsert_sql"
    ...
    class << self
      ...
      def increment!(event)
        ...
        LlmCostTracker::PeriodTotal.upsert_all(
          period_rows(event),
          on_duplicate: RollupUpsertSql.call(LlmCostTracker::PeriodTotal),
          record_timestamps: true,
          unique_by: unique_by(LlmCostTracker::PeriodTotal, %i[period period_start])
        )
      end
      ...
      def period_rows(event)
        Periods::PERIODS.map do |period, name|
          {
            period: name,
            period_start: Periods.bucket(period, event.tracked_at),
      ...
      def period_decrement_totals(call_rows)
        call_rows.each_with_object(Hash.new { |totals, key| totals[key] = BigDecimal("0") }) do |row, totals|
          ...
          Periods::PERIODS.each_key do |period|
            totals[[period, Periods.bucket(period, tracked_at)]] += BigDecimal(total_cost.to_s)
          end
        end
      end
      ...
      def apply_decrements(totals)
        ...
        totals.each do |(period, period_start), amount|
          row = LlmCostTracker::PeriodTotal.lock.find_by(period: Periods::PERIODS.fetch(period),
      ...
      def rollup_period_totals(periods, time)
        buckets = periods.to_h { |period| [period, Periods.bucket(period, time)] }
        index = buckets.to_h { |period, bucket| [[Periods::PERIODS.fetch(period), bucket], period] }
        totals = periods.to_h { |period| [period, 0.0] }

        LlmCostTracker::PeriodTotal
          .where(period: periods.map { |period| Periods::PERIODS.fetch(period) },
      ...
      def fallback_period_total(period, time)
        LlmCostTracker::LlmApiCall
          .where(tracked_at: Periods.range_start(period, time)..time)
          .sum(:total_cost)
          .to_f
      end
