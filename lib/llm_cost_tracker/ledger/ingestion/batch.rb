  ...
  class Ledger
    module Ingestion
      class Batch
        ...
        def pending?
          Ledger::Ingestion::Event.where("attempts < ?", Ledger::Ingestion::Inbox::MAX_ATTEMPTS).exists?
        end
        ...
        def claim
          ...
          Ledger::Ingestion::Event.transaction do
            rows = claimable_scope(cutoff).order(:id).limit(BATCH_SIZE).lock.to_a
            ids = rows.map(&:id)
            next [] if ids.empty?

            updates = Ledger::Ingestion::Event.sanitize_sql_array(
              ["locked_at = ?, locked_by = ?, attempts = attempts + 1, updated_at = ?", now, identity, now]
            )
            Ledger::Ingestion::Event.where(id: ids).update_all(updates)
            Ledger::Ingestion::Event.where(id: ids, locked_by: identity).order(:id).to_a
          end
        end
        ...
        def decode(rows)
          ...
          events = []
          rows.each do |row|
            events << Ledger::Ingestion::Inbox.event_from_row(row)
            valid_rows << row
          rescue StandardError => e
            mark_failed([row], e)
          end
        ...
        def persist(rows, events)
          LlmCostTracker::Ledger::Call.transaction do
            ...
            Ledger::Ingestion::Event.where(id: rows.map(&:id), locked_by: identity).delete_all
          end
        end

        def claimable_scope(cutoff)
          Ledger::Ingestion::Event
            .where("attempts < ?", Ledger::Ingestion::Inbox::MAX_ATTEMPTS)
