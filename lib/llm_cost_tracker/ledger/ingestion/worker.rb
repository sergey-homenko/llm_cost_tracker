  ...
  class Ledger
    module Ingestion
      class Worker
        ...
        class << self
          def ensure_started
            return unless Ledger::Ingestion::Inbox.enabled?
          ...
          def flush!(timeout: FLUSH_TIMEOUT_SECONDS, require_lease: false)
            return true unless Ledger::Ingestion::Inbox.enabled?

            deadline = Time.now.utc + timeout
            loop do
              return true unless Ledger::Ingestion::Batch.new(identity: identity).pending?
          ...
          def ingest_once(require_lease: true)
            return 0 unless Ledger::Ingestion::Inbox.enabled?

            batch = Ledger::Ingestion::Batch.new(identity: identity)
            return 0 unless batch.claimable?
            return 0 if require_lease && !Ledger::Ingestion::LeaseClaim.new(identity: identity,
                                                                            seconds: LEASE_SECONDS).acquire
