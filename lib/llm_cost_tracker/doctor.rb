...
module LlmCostTracker
  class Doctor
    ...
    def call_rollups_check
      return unless llm_cost_tracker_calls_table?
      return live_rollups_check unless LlmCostTracker.configuration.cache_period_totals
    ...
    def live_rollups_check
      if Probe.table_exists?("llm_cost_tracker_call_rollups")
        Check.new(
          :warn,
          "call rollups",
          "cache_period_totals=false but llm_cost_tracker_call_rollups exists. " \
          "Set config.cache_period_totals = true to keep budget reads on the rollups fast path or drop the table."
        )
      else
        Check.new(
          :ok,
          "call rollups",
          "cache_period_totals=false; budget reads aggregate from llm_cost_tracker_calls directly"
        )
      end
    end
