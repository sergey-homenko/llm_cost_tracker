  ...
  module Providers
    module Openai
      module ReconciliationSource
        FINGERPRINT_KEYS = %i[start_time end_time line_item project_id api_key_id].freeze
        ROW_TYPE_COST = "cost"
        ...
        def self.row_for_result(raw, period_start:, period_end:, start_time:, end_time:, authority:, row_type:)
          ...
          fingerprint = fingerprint_for(result, start_time: start_time, end_time: end_time)
          {
            ...
            currency: (amount[:currency] || LlmCostTracker::Billing::DEFAULT_CURRENCY).to_s.upcase,
