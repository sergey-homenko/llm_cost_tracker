  ...
  Event = Data.define(
    ...
    def self.build(**attributes)
      ...
      new(
        ...
        model: clean(attributes.fetch(:model)) || Event::UNKNOWN_MODEL,
        ...
        provider_response_id: clean(attributes[:provider_response_id]),
        provider_project_id: clean(attributes[:provider_project_id]),
        provider_api_key_id: clean(attributes[:provider_api_key_id]),
        provider_workspace_id: clean(attributes[:provider_workspace_id]),
    ...
    def self.clean(value)
      Ledger::Storable.text(value.to_s).strip.presence
    end
