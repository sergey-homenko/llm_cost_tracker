...
| RubyLLM embed / transcribe | `RubyLLM::Provider#embed`, `#transcribe`, plus `RubyLLM::Providers::Gemini::Transcription#transcribe` on RubyLLM 1.x, where Gemini and Vertex AI transcription bypass `Provider#transcribe` | Routed through the matched provider parser; priced like the underlying provider call |
