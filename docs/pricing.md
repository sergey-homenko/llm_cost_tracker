...
OpenAI Realtime does report the split: `input_token_details.audio_tokens` and `image_tokens` include their cached share, and `cached_tokens_details` breaks `cached_tokens` down by modality. The parser takes the cached share out of `audio_input` and `image_input`, so every input token lands in exactly one bucket, and all cached tokens are billed at `cache_read_input`. On the full-size `gpt-realtime` models that is exact for cached audio ($0.40 / M, the same as cached text) and slightly low for cached images ($0.50 / M published). The mini models (`gpt-realtime-mini`, `gpt-realtime-2.1-mini`) publish cached audio at $0.30 / M against $0.06 / M for cached text, so their cached audio is under-priced.
...
| OpenAI Realtime `response.done` | Provider stream events passed through `track_stream`; standard Faraday middleware does not auto-capture WebSocket/WebRTC sessions | Audio input/output token rates price the call when the model has registry rates; cached text, audio, and image input is priced at `cache_read_input` |
