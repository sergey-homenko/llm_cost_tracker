...
Refresh refuses a snapshot that looks broken or tampered with and leaves the local file as it was. It stops when the snapshot sets a price that was above zero to zero, charges for something that was free, removes a model's `input` or `output` rate while keeping the model, moves any existing price 100-fold or more in either direction, drops more than a third of the models the file got from earlier refreshes, or switches the currency. The error lists them. `PREVIEW=1` prints the same list without writing, and so does `prices:check`. Once you have confirmed the changes are real, run the refresh again with `FORCE=1`, or pass `force: true` to `LlmCostTracker::Pricing::Sync.refresh`.

New models, removed models below that share, added or dropped optional rates such as batch or cache tiers, and moves short of 100-fold go through without a prompt. New models are not checked at all, so a new, more specific key such as a dated snapshot id can still change what an existing model costs. Entries marked `_source: manual` that the snapshot does not also list survive a refresh and don't count as removed; a snapshot entry with the same key replaces them. A first refresh into a missing or empty file has nothing to compare against and is never refused.

The limits come from the bundled price history since v0.6, when model keys gained their provider prefix. The largest move any price made between two snapshots was about 55-fold, on an OpenRouter cache-read rate; first-party list prices never moved more than 5-fold; and no model price was ever set to zero. A 10-fold limit would have stopped most refreshes a month apart, because OpenRouter rates swing that much. The checks are a sanity check against a snapshot that zeroes existing prices or inflates them by orders of magnitude, not a signature, so keep reviewing the refreshed file before you ship it.
