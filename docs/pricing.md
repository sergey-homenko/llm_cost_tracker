...
- Precedence is `pricing.overrides`, then `pricing.file`, then bundled prices. The first matching entry is used whole; rates are not merged across sources, so an override must list every rate its calls use. A billed `usage.cost` in the response (see Provider-Billed Cost) is used ahead of all three.
...
- A model with no key under its own provider takes the price of the only `provider/model` key with the same model name, so Azure OpenAI's `gpt-4o` prices as `openai/gpt-4o`. A dated id with no key of its own (`-YYYY-MM-DD`, `-YYYYMMDD`, or `-preview-`/`-exp-` plus `MM-DD` or `MM-YYYY`) takes the price of its undated key, so `claude-sonnet-4-5-20250929` prices as `anthropic/claude-sonnet-4-5`. With no match the call is recorded as `cost_status: unknown`.
...
| Gemini grounding | `groundingMetadata.webSearchQueries` | Priced from the model's own `grounding_request` rate, which Google publishes per 1,000 requests and differs by family ($35 per 1,000 grounded prompts on Gemini 2.x, $14 per 1,000 unique non-empty search queries on 3.x). The free monthly allowance is account-level and is not modelled, so a project inside it is over-reported. Grounding with Google Maps has no line item of its own: a response that carries `webSearchQueries` is priced as Search grounding (on Gemini 2.x at $35 per 1,000 instead of Maps' $25; the 3.x rates match), and one without them is not charged |
...
The Faraday middleware, which `ruby-openai` uses, cannot read the model from a multipart body, so OpenAI transcriptions and image edits and variations sent through it are recorded with model `unknown` and no cost when the response carries usage, and translations and `/audio/speech`, which return none, are not recorded. The official `openai` gem integration captures these endpoints.
