...
| Table | Role |
| --- | --- |
| `llm_api_calls` | Parent ledger row for one tracked LLM call |
...
| `llm_cost_tracker_inbox_events` | Durable staging rows before ledger insertion |
| `llm_cost_tracker_ingestor_leases` | Shared worker lease rows for durable ingestion |
...
| `cache_write_1h_input_tokens` | integer, not null, default `0` | 1-hour cache-write input tokens |
...
| `cache_write_1h_input_cost` | decimal, precision `20`, scale `8` | 1-hour cache-write cost |
...
| --- | --- | --- |
| `id` | primary key | Rails default |
| `llm_api_call_id` | bigint, not null | Parent `llm_api_calls` id |
...
| Index | Purpose |
| --- | --- |
| `llm_api_call_id` | Call details and dependent pruning |
...
this table instead of scanning `llm_api_calls`.
...
| Field | Stored data |
| --- | --- |
| `llm_api_calls.tags` | Sanitized application attribution tags |
| `llm_api_calls.pricing_snapshot` | Schema version, source metadata, currency, and applied token rates |
| `llm_cost_tracker_service_charges.details` | Provider item details used for audit and reconciliation |
| `llm_cost_tracker_inbox_events.payload` | Versioned event payload for durable ingestion |
