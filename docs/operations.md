...
- Automatic capture never fails an LLM call because recording failed. Only `BudgetExceededError`, `UnknownPricingError` under `pricing.unknown_model_behavior = :raise`, and `TransactionAbortedError` reach your code; any other recording failure is logged. `LlmCostTracker.track` and `track_stream` raise recording errors, because you called them to record — except that when your `track_stream` block raised, its exception wins and the recording error is logged, unless it is a `TransactionAbortedError`.
