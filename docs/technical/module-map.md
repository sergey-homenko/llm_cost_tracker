...
- Maintain call rollups for hot-path budget reads when `config.`config.budgets.totals_source = :cache``; otherwise budget reads aggregate live from `llm_cost_tracker_calls`.
...
- Keep daily and monthly call rollups consistent when `config.`config.budgets.totals_source = :cache``.
