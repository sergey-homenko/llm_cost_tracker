...
3. `Ledger::Store.insert` writes the call header, line items, tag rows (and rollup increments when `config.budgets.totals_source = :cache``), and inbox deletes in one transaction.
