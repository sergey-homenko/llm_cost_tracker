...
6. The persisted event is emitted through `ActiveSupport::Notifications`. With `pricing.unknown_model_behavior = :raise`, an event with no rate then raises `LlmCostTracker::UnknownPricingError`, after it is recorded.
